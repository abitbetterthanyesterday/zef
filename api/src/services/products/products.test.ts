...
describe('products', () => {
  ...
  scenario('creates a product', async (scenario: StandardScenario) => {
    const result = await createProduct({
      input: {
        name: 'String',
        brandId: scenario.product.two.brandId,
    ...
    expect(result.brandId).toEqual(scenario.product.two.brandId)
    expect(result.categoryId).toEqual(scenario.product.two.categoryId)
    expect(result.updatedAt.toISOString()).toEqual('2022-11-07T03:13:54.000Z')
  })
